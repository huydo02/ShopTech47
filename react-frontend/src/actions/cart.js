...
export const addToCart = (data) => async (dispatch) => {
    try {
        const result = await fetch(`http://localhost:3001/cart/add/${data.productId}`, {
            method: "POST",
            headers: {
...
export const getCartItem = () => async (dispatch) => {
    try {
        const result = await fetch("http://localhost:3001/cart/cartQuantity", {
            method: "GET",
            credentials: "include",
        })
            .then(res => res.json())
...
export const updateCart = (data) => async (dispatch) => {
    try {
        const result = await fetch(`http://localhost:3001/cart/update/${data.productId}/${data.quantity}`, {
            method: "PATCH",
            headers: {
                "Content-Type": "application/json",
            },
...
export const removeCart = (productId) => async (dispatch) => {
    try {
        const result = await fetch(`http://localhost:3001/cart/delete/${productId}`, {
            method: "DELETE",
            headers: {
                "Content-Type": "application/json",
            },
