...
const Build = () => {
    ...
    const fetchApi = async () => {
        try {
            const response = await fetch(`http://localhost:3001/build/categories`, {
                credentials: "include",
                method: "GET",
            });
    ...
    const fetchSort = async (child, filter = '') => {
        const response = await fetch(`http://localhost:3001/build/categories/${child}?filter=${filter}`, {
            credentials: "include",
            method: "GET",
        });
    ...
    const handleSetdataBuild = async (dataproduct) => {
        ...
        if (dataproduct) {
            ...
            const response = await fetch(`http://localhost:3001/build/add/`, {
                credentials: "include",
                method: "POST",
                headers: {
    ...
    const handleRemove = async (dataproduct) => {
        ...
        const response = await fetch(`http://localhost:3001/build/remove/`, {
            credentials: "include",
            method: "DELETE",
            headers: {
                "Content-Type": "application/json",
    ...
    const hanldeAddToCart = async () => {
        if (databuild.length > 0) {
            try {
                const response = await fetch(`http://localhost:3001/build/add-to-cart`, {
                    credentials: "include",
                    method: "GET",
                });
                const data = await response.json();
    ...
    const handleRemoveBuild = async () => {
        if (databuild.length > 0) {
            try {
                const response = await fetch(`http://localhost:3001/build/remove-build`, {
                    credentials: "include",
                    method: "DELETE",
                });
                const data = await response.json();
