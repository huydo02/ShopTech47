...
const Products = () => {
    ...
    const fetchApi = async () => {
        await fetch(`http://localhost:3001/admin/list-product?${qs.stringify(getRandomuserParams(tableParams))}`)
            .then(res => res.json())
            .then(data => {
    ...
    const handleDelete = async (record) => {
        try {
            const response = await fetch(
                `http://localhost:3001/admin/list-product/delete-product/${record._id}`,
                { method: 'DELETE' }
            );
    ...
    const handleUpdate = async (record) => {
        ...
        try {
            const response = await fetch(
                `http://localhost:3001/admin/list-product/change-status/${newStatus}/${record._id}`,
                { method: 'PATCH' }
            );
            const result = await response.json();
            if (result.status === 'success') {
