...
const UpdateProduct = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const data = await fetch(`http://localhost:3001/admin/list-product/edit/${id_product.id}`);
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            const res = await fetch(`http://localhost:3001/admin/list-product/edit/${id_product.id}`, {
                method: "PATCH",
                body: formData,
            });
